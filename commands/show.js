          ...
          if (char != undefined) {
            ...
            if (args[0].includes('slot') && args.length < 2) {
              ...
              if(args[0] === 'arma' || args[0] === 'escudo' || args[0] === 'armadura'){
                message.channel.send(renderShowItem(char.equipedItems[args[0]])).then((msg) => msg.delete({ timeout: 20000 }));
              } else {
                message.channel.send(renderShowItem(char.backpack[args[0]])).then((msg) => msg.delete({ timeout: 20000 }))
              }
            } else {
              message.reply('Defina um slot que você deseja inspecionar (slot1..., arma, escudo ou armadura).').then((msg) => msg.delete({ timeout: 20000 }))
            }
